...
@Service
public class JobArticleServiceImpl implements JobArticleService {
...
//                        更新图片url
                    int updateImg = jobArticleMapper.update(article);
                    if (updateImg<=0){
...
//            判断是否需要处理图片
            if(imageHolder != null){
//            若存在图片，则取出相应文章
                JobArticle oldArticle = jobArticleMapper.findArticleById(article.getId());
...
//            删除文章所在路径
            ...
            String delePath = "/upload/item/"+userStr+"/jobArticle/"+lostArticleStr;
//            删除路径对应文件
...
//                删除该文章评论
                ...
                comment.setArticleParCategory(4);
...
//                删除该文章
    ...
    private void addArticleImg(JobArticle article, ImageHolder ArticleInputStream){
        //获取图片目录的相对值路径
        //通过userid,articleid划分文件地址，并传入下一个函数作为参数
        String dest = PathUtil.getJobArticleImagePath(article.getUserInfo(),article.getId());
