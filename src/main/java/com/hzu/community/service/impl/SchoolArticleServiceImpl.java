...
public class SchoolArticleServiceImpl implements SchoolArticleService {
    ...
    @Override
    @Transactional
    public ArticleEnum saveArticle(SchoolArticle article, ImageHolder imageHolder) throws ArticleException {
        if (article == null) {
        ...
        try {
            ...
            int insertNum = schoolArticleMapper.add(article);
...
//                        更新图片url
                    int updateImg = schoolArticleMapper.update(article);
                    if (updateImg<=0){
    ...
    @Override
    @Transactional
    public ArticleEnum updateArticle(SchoolArticle article, ImageHolder imageHolder) throws ArticleException {
        try {
//            判断是否需要处理图片
            if(imageHolder != null){
//            若存在图片，则取出相应文章
                SchoolArticle oldArticle = schoolArticleMapper.findArticleById(article.getId());
...
//           更新文章信息
            article.setEditTime(new Date());
            int updateNum = schoolArticleMapper.update(article);
            if (updateNum<=0){
...
//                删除该文章下的信息通知
                ...
                notification.setArticleParCategory(5);
...
//                删除该文章评论
                ...
                comment.setArticleParCategory(5);
            ...
            try {
//                删除该文章
                schoolArticleMapper.deleById(id);
            }catch (Exception e){
...
//            删除路径对应文件
    ...
    private void addArticleImg(SchoolArticle article, ImageHolder ArticleInputStream){
        //获取图片目录的相对值路径
        //通过userid,articleid划分文件地址，并传入下一个函数作为参数
        String dest = PathUtil.getSchoolArticleImagePath(article.getUserInfo(),article.getId());
